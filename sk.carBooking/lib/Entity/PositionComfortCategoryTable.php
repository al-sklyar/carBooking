...
class PositionComfortCategoryTable
{
    ...
    public static function getEntity()
    {
        if (self::$entity === null) {
            $hlblock = HL\HighloadBlockTable::getList([
                'filter' => ['TABLE_NAME' => 'sk_position_comfort_categories']
            ])->fetch();

            if ($hlblock) {
                self::$entity = HL\HighloadBlockTable::compileEntity($hlblock)->getDataClass();
            } else {
                throw new \Exception('Highload block sk_position_comfort_categories не найден');
            }
        }
